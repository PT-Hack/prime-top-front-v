...
<template>
  <div class="bg-white rounded-lg border border-gray-400 hover:shadow-lg transition-all cursor-pointer overflow-hidden"
    @click="goToProduct">
    <!-- Изображение товара -->
    <div class="relative bg-gray-200 aspect-square flex items-center justify-center">
      <img src="@/assets/vedro1.png" alt="Изображение товара" class="w-full h-full object-cover">
      <!-- Кнопка избранного -->
      <button v-if="canAddToFavorites" @click="toggleFavorite" :disabled="favoriteLoading"
      ...
      <!--Цвет-->
      <div class="absolute right-1 top-1 w-10 h-10 rounded-full border-1 border-gray-400"
    ...
    <div class="p-4">
      ...
      <div v-if="product.series && product.series.length > 0" class="mb-2">
        <div v-if="product.series.length === 1" class="space-y-1">
          <div class="flex items-center justify-between">
            ...
            <span class="text-xs font-medium">{{ product.color_ral_code || 'Без названия' }}</span>
          </div>
          <div class="flex items-center justify-between">
      ...
      <button v-if="canAddToCart && firstSeries" @click="addSeriesToCart"
