...
const fullName = computed(() => formatters.fullName(currentUser.value))
const userInitial = computed(() => formatters.initial(currentUser.value))
...
const handleNotificationChange = async () => {
  ...
  try {
    // В реальном приложении здесь будет обновление настроек уведомлений
        ...
        <div class="lg:col-span-1">
          <AppCard>
            <div class="text-center">
              ...
              <h2 class="text-xl font-bold text-text mb-1">{{ fullName }}</h2>
              ...
              <div class="inline-block px-3 py-1 bg-primary-100 text-primary text-sm font-medium rounded-full mb-4">
                {{ currentUser?.role?.description || currentUser?.role?.title || 'Пользователь' }}
              </div>
